      ...
      <div className="section-shell">
        <div className="flex flex-col justify-between gap-5 md:flex-row md:items-end">
          <div className="max-w-2xl">
            ...
            <p className="mt-4 text-base leading-7 text-ink/68">
              Cada curso puede tener foros activos o archivados. Al llegar al tercer nivel,
              el hilo continua con seguimiento y likes, sin crear mas profundidad.
            </p>
          </div>
        ...
        <div className="mt-10 grid gap-5 lg:grid-cols-3">
          {courseForums.map((forum) => (
            <article className="rounded border border-ink/10 bg-paper p-5 shadow-soft" key={forum.id}>
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-sm font-bold text-copper">
                    {forum.courseCode} · {forum.courseName}
                  </p>
                  <h3 className="mt-2 text-xl font-bold leading-7">{forum.title}</h3>
                </div>
              ...
              <p className="mt-4 text-sm leading-7 text-ink/68">{forum.excerpt}</p>
                  ...
                  >
                    ...
                    <div className="mt-3 flex flex-wrap gap-2 text-xs font-bold">
                      <button
                        ...
                        onClick={() => likeReply(forum.id, reply.id)}
                      >
                        <Heart size={13} />
                        {reply.likes}
                      </button>
                      {reply.level < 3 ? (
                        <button
                          ...
                          onClick={() => setTargets((current) => ({ ...current, [forum.id]: reply.id }))}
                        >
                          <Reply size={13} />
                          Responder
                        </button>
              ...
              {forum.status === "Abierto" ? (
                <div className="mt-4 grid gap-3 border-t border-ink/10 pt-4">
                  <input
                    ...
                    value={authors[forum.id] ?? ""}
                    onChange={(event) =>
                      setAuthors((current) => ({ ...current, [forum.id]: event.target.value }))
                    }
                  />
                  <select
                    className="h-10 rounded border border-ink/12 bg-white px-3 text-sm outline-none"
                    ...
                    onChange={(event) =>
                      setTargets((current) => ({ ...current, [forum.id]: event.target.value }))
                    }
                  >
